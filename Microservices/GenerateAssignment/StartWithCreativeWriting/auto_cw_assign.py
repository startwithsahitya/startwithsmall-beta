...
def generate_case_code():
    ...
    try:
        with open("GenerateAssignment\creative_writing\last_case_code.txt", "r") as file:
    ...
    with open("GenerateAssignment\creative_writing\last_case_code.txt", "w") as file:
        file.write(str(new_code))

    # Format the case code as CWXXX
    return f"CW{new_code:03}"

# Function to generate company name
def generate_company_name():
    # Explicitly ask for only one company name
    response = model.generate_content("Generate only one company name, that sounds like the company is a creative writing company,  with maximum 2 words.")
...
def generate_company_description():
    response = model.generate_content("Generate a company description, the description should throw light on the company's creative writing background, in maximum 70 words.")
    return response.text.strip()

# Function to generate job description (problem statement)
def generate_job_description():
    response = model.generate_content("Generate a case study that equates to a problem statement which is based on creative writing, such as writing a news report or a blog on a specific topic or  job posting for the company based on parameters in maximum 100 words. Do not give any headings.")
    return response.text.strip()
...
def store_in_database(case_code, company_name, company_description, case_study):
    try:
        ...
        sql = "INSERT INTO creative_writing_assignments (case_code, company_name, company_description, case_study) VALUES (%s, %s, %s, %s)"
